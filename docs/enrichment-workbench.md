...
Kimi API key env: MOONSHOT_API_KEY
...
The Kimi profile can point at `MOONSHOT_API_KEY`; the proxy also checks `KIMI_API_KEY` and `KIMI_OPEN_PLATFORM_API_KEY` as fallbacks.
...
Each image is checksummed before processing. New images are uploaded to S3 under a UUID resource directory, tiled into a IIIF Level 0 image service, given a 512-ish thumbnail, sent through historical map extraction, passed through a second Aardvark metadata-writing step, published into DuckDB, and written back to S3 as Aardvark JSON. If a Google Vision OCR profile is selected in the Upload tab, Google Vision performs text extraction and bounding-box generation first. A selected label-reconciliation profile can then inspect OCR source crops with OpenAI mini models, Gemini, or Kimi K2.6. The Kimi path runs a gated map-agent swarm that emits both visible map labels and compact structured claims for layout, title/date/publisher, legend/subject, scale/projection, OCR repair, false-positive suppression, extent, temporal coverage, Aardvark validation, and human-review packets. The proxy fuses accepted labels with Google Vision OCR, filters obvious over-merged OCR groups, preserves Kimi `prompt_cache_key` values and response-cache hit counts, and carries Kimi claims into `ai-enrichments.json` under `extensions.kimiSwarm`. Kimi street labels remain searchable map text; only metadata-grade features such as parks, waterbodies, neighborhoods, landmarks, landforms, and railroads become derived placenames for gazetteer matching. The built-in Kimi profile uses `kimi-k2.6`, `MOONSHOT_API_KEY`, thinking disabled, structured JSON output, and a local response cache at `web/.cache/kimi-agent-swarm/responses`; set `KIMI_AGENT_SWARM_RESPONSE_CACHE=false` to disable that local cache. Select this profile explicitly in the label-reconciliation dropdown when you want the Kimi swarm path. See [Kimi Map-Agent Swarm Pipeline](kimi-map-agent-swarm.md) for the detailed agent, caching, provenance, and review workflow. If no label-reconciliation profile is selected, OpenAI can still run the legacy vision augmentation pass over the image derivatives to catch curved, faint, stylized, rotated, or otherwise missed map labels before using the merged evidence to prepare the OpenGeoMetadata Aardvark record. Set `OPENAI_VISION_AUGMENT_OCR_ENABLED=false` to disable that additive pass. Previously processed checksums reuse the existing S3 artifacts while refreshing the local DuckDB resource. If a Google Vision OCR profile is selected and the cached extraction was not produced by Google Vision, the proxy refreshes `enrichment_response.json` with Google Vision before rewriting Aardvark.

During long uploads, the browser polls the proxy job progress endpoint and surfaces live milestones such as Kimi crop queue size, completed crop count, labels, claims, cache hits, and downstream metadata-writing phases. This progress stream is separate from the final upload response, so the Upload tab should show actual proxy work instead of only a generic waiting heartbeat.
