...
describe('ImportPage', () => {
  ...
  it('renders welcome message when resourceCount is 0', () => {
    ...
    expect(
      screen.getByText(/Welcome to Aardvark Metadata Studio/)
    ).toBeDefined();
  });
  ...
  it('does not render welcome message when resources exist', () => {
    ...
    expect(
      screen.queryByText(/Welcome to Aardvark Metadata Studio/)
    ).toBeNull();
  });
  ...
  it('handles JSON import', async () => {
    ...
    await waitFor(() => {
      ...
      expect(
        screen.getByText(/Import complete! Loaded 10 resources/)
      ).toBeDefined();
    });
  });
  ...
  it('handles CSV import', async () => {
    ...
    await waitFor(() => {
      ...
      expect(
        screen.getByText(/Import complete! Loaded 5 resources/)
      ).toBeDefined();
    });
  });

  it('handles export JSON zip', async () => {
    vi.mocked(duckdb.exportAardvarkJsonZip).mockResolvedValue(
      new Blob(['zip data'])
    );
  ...
  it('exports with the selected AGSL filename profile', async () => {
    ...
    expect(window.localStorage.getItem('aardvark-json-filename-profile')).toBe(
      'agsl'
    );
  });

  it('shows an in-app validation report for invalid AGSL exports', async () => {
    const onEdit = vi.fn();
    vi.mocked(duckdb.exportAardvarkJsonZip).mockRejectedValue(
    ...
    render(<ImportPage onEdit={onEdit} />);
    ...
    expect(onEdit).toHaveBeenCalledWith('ark:-77981-gmgscj87k49');
  });
