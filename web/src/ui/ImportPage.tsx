...
export const ImportPage: React.FC<ImportPageProps> = ({
  ...
  const handleExportJsonZip = async () => {
    try {
      ...
      if (err instanceof ExportValidationError) {
        setExportValidationError(err);
        setStatus('');
      } else {
        setStatus(
          `Export failed: ${err instanceof Error ? err.message : 'Unexpected export failure'}`
        );
      }
    } finally {
      setLoading(false);
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setStatus(
        `Import complete! Loaded ${totalRows} resources. Data saved to in-memory DB and IndexedDB.`
      );
    } catch (err: any) {
      console.error(err);
  ...
  const handleSaveDb = async () => {
    try {
      ...
      if (blob) {
        ...
        URL.revokeObjectURL(url);
        setStatus(
          'Database downloaded. Please commit this file to the repository.'
        );
      }
    } catch (err: any) {
        ...
        {resourceCount === 0 && (
          <div className="bg-gradient-to-r from-indigo-500 to-purple-600 rounded-xl p-8 text-white shadow-lg animate-fade-in-up">
            <h1 className="text-3xl font-bold mb-4">
              Welcome to Aardvark Metadata Studio!
            </h1>
            <p className="text-lg opacity-90 max-w-2xl">
              It looks like your database is empty. To get started, please
              import some data below. You can upload CSV/JSON files or connect a
              GitHub repository.
            </p>
          </div>
        )}
        ...
        {mode === 'local' && (
          <div className="bg-white dark:bg-slate-900 rounded-lg border border-gray-200 dark:border-slate-800 p-6 shadow-sm">
            <h2 className="text-lg font-semibold mb-4 text-slate-900 dark:text-slate-200">
              1. CSV / JSON / DuckDB Import
            </h2>
            <p className="text-slate-500 dark:text-slate-400 mb-4 text-sm">
              Upload Aardvark-compliant CSV files, OGM Aardvark JSON files, or a{' '}
              <b>.duckdb</b> backup file. Existing records with matching IDs
              will be updated (CSV/JSON) or replaced (DB Backup).
            </p>
            <input
              type="file"
            ...
            {status && (
              <div
                className={`mt-6 p-4 rounded-md ${status.startsWith('Error') ? 'bg-red-50 dark:bg-red-900/50 text-red-700 dark:text-red-200 border-red-200 dark:border-red-800' : 'bg-gray-50 dark:bg-slate-800 text-slate-700 dark:text-slate-200 border-gray-200 dark:border-slate-700'} border`}
              >
                {status}
              </div>
            )}
