...
import {
  ...
  useJsonExportProfile,
} from './JsonExportProfileSelect';
...
import { ExportValidationError } from '../aardvark/validation';
...
export const Dashboard: React.FC<DashboardProps> = ({ onEdit, onSelect }) => {
  ...
  const [exportValidationError, setExportValidationError] =
    useState<ExportValidationError | null>(null);
  ...
  const currentBBox = useMemo(() => {
    ...
    if (p.length === 4 && p.every((n) => !isNaN(n)))
      return { minX: p[0], minY: p[1], maxX: p[2], maxY: p[3] };
    return undefined;
  }, [state.bbox]);
  ...
  const handleExport = async (format: 'json' | 'csv') => {
    ...
    try {
      ...
      const jsonOptions =
        format === 'json' ? jsonExportProfile.options : undefined;
      ...
      if (e instanceof ExportValidationError) {
        setExportValidationError(e);
      } else {
        console.error('Export failed', e);
        setExportError(
          e instanceof Error ? e.message : 'Unexpected export failure'
        );
      }
    } finally {
    ...
    <div className="flex bg-gray-50 dark:bg-slate-900 h-full transition-colors duration-200">
      {exportValidationError && (
        <ExportValidationReport
          ...
          onClose={() => setExportValidationError(null)}
          onEdit={onEdit}
        />
      )}
      {/* Sidebar: Facets */}
      <div className="hidden md:block w-96 flex-shrink-0 border-r border-gray-200 dark:border-slate-800 p-4 overflow-y-auto bg-white dark:bg-transparent">
        ...
        <div className="space-y-4">
          {FACETS.filter((f) => f.field !== 'gbl_indexYear_im').map(
            (f, index) => {
              ...
              return (
                <FacetSection
                  ...
                  onShowMore={
                    hasMore
                      ? () => setModalState({ field: f.field, label: f.label })
                      : undefined
                  }
                />
              );
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <div className="z-10 relative border-b border-gray-200 dark:border-slate-800 bg-white/50 dark:bg-slate-900/50 p-4 flex flex-col gap-4 backdrop-blur-sm">
          {exportError && (
            <div
              role="alert"
              className="flex items-center justify-between gap-4 rounded-md border border-red-200 bg-red-50 px-4 py-2 text-sm text-red-800 dark:border-red-900 dark:bg-red-950/40 dark:text-red-200"
            >
              ...
              <div className="flex bg-gray-100 dark:bg-slate-800 rounded-md p-0.5 border border-gray-200 dark:border-slate-700 mr-2">
                ...
                >
                  ...
                  >
                    <path
                      fillRule="evenodd"
                      d="M2.5 4.75A.75.75 0 013.25 4h14.5a.75.75 0 010 1.5H3.25A.75.75 0 012.5 4.75zm0 10.5a.75.75 0 01.75-.75h7.5a.75.75 0 010 1.5h-7.5a.75.75 0 01-.75-.75zM2.5 10a.75.75 0 01.75-.75h14.5a.75.75 0 010 1.5H3.25A.75.75 0 012.5 10z"
                      clipRule="evenodd"
                    />
                  </svg>
                </button>
                <button
                  onClick={() =>
                    setState((prev) => ({ ...prev, view: 'gallery' }))
                  }
          ...
          <ActiveFilterBar
            ...
            onRemoveYearRange={() =>
              setState((prev) => ({ ...prev, yearRange: undefined, page: 1 }))
            }
            onClearAll={() =>
              setState((prev) => ({
                ...prev,
        ...
        {view === 'map' ? (
          <div className="flex-1 flex items-start">
            ...
            <div className="flex-1 sticky top-[88px] h-[calc(100vh-100px)]">
              {loading ? (
                <div className="absolute inset-0 flex items-center justify-center bg-gray-50/50 dark:bg-slate-900/50 backdrop-blur-sm z-10">
                  Loading...
                </div>
              ) : (
                <ResultsMapView
          ...
          <div className="flex-1 overflow-y-auto p-4">
            {loading && resources.length === 0 ? (
              ...
              <DashboardResultsList
                ...
                mapUrls={mapUrls}
                onSelect={onSelect}
                onAddFilter={(f, v) => toggleFacet(f, v, 'include')}
                page={page}
              />
            )}
          </div>
            ...
            return (
              ...
              >
                <button
                  ...
                  className={`flex-1 flex items-center text-sm cursor-pointer py-0.5 text-left min-w-0 ${
                    isIncluded
                ...
                >
                  ...
                  <span className="ml-2 text-xs text-slate-400 dark:text-slate-600 font-mono flex-shrink-0">
                    {item.count}
                  </span>
                </button>
