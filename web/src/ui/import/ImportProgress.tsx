...
export const ImportProgress: React.FC<ImportProgressProps> = ({
  ...
  return (
    <div className="space-y-4 pt-4 border-t border-gray-200 dark:border-slate-800">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-slate-500">
            Detected Schema:{' '}
            <span className="font-semibold uppercase">{schemaMode}</span>
          </p>
        </div>
        {!isImporting && (
          <div className="flex gap-2">
            ...
            {onRebuild && (
              <button
                onClick={onRebuild}
                className="px-4 py-2 bg-indigo-600 hover:bg-indigo-500 text-white rounded text-sm font-medium shadow-sm"
              >
      ...
      {onRebuild && !isImporting && (
        <p className="rounded border border-indigo-200 bg-indigo-50 p-3 text-xs text-indigo-800 dark:border-indigo-900 dark:bg-indigo-950/30 dark:text-indigo-200">
          Rebuild downloads and checks every JSON file before atomically
          replacing the derived Studio database. If preparation or replacement
          fails, the existing database is preserved.
        </p>
      )}
      ...
      {resultMessage && (
        <div
          role="status"
          className="rounded border border-slate-200 bg-slate-50 p-3 text-sm text-slate-700 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-200"
        >
          {resultMessage}
        </div>
      )}
        ...
        <div className="space-y-2">
          <div className="h-2 w-full bg-gray-200 dark:bg-slate-800 rounded-full overflow-hidden">
            <div
              ...
              style={{
                width: `${(progress.current / foundFiles.length) * 100}%`,
              }}
            />
          </div>
          <div className="flex justify-between text-xs text-slate-500 dark:text-slate-400">
            <span>
              Processing: {progress.current} / {foundFiles.length}
            </span>
            <span>
              Success: {progress.successes} | Fail: {progress.failures}
            </span>
          </div>
        </div>
      )}
      ...
      {errorLogs.length > 0 && (
        <div className="max-h-64 overflow-y-auto border border-red-200 dark:border-red-900 rounded bg-red-50 dark:bg-red-950/20 p-2 text-xs font-mono text-red-600 dark:text-red-300">
          <div className="font-bold border-b border-red-200 dark:border-red-900 mb-2 pb-1">
            Error Log (Last 100)
          </div>
          {errorLogs.map((e, i) => (
      ...
      <div className="max-h-64 overflow-y-auto border border-gray-200 dark:border-slate-800 rounded bg-gray-50 dark:bg-slate-900/50 p-2 text-xs font-mono text-slate-500 dark:text-slate-400">
        {foundFiles.slice(0, 100).map((f) => (
          <div key={f.sha} className="truncate">
            {f.path}
          </div>
        ))}
        {foundFiles.length > 100 && (
          <div className="italic pt-2">
            ...and {foundFiles.length - 100} more
          </div>
        )}
      </div>
    </div>
