...
describe('Missing Components Coverage', () => {
  ...
  describe('MapFacet', () => {
    it('renders map and handles search', () => {
      ...
      expect(onChange).toHaveBeenCalledWith(
        expect.objectContaining({ minX: 0 })
      );
    });
    ...
    it('renders Clover for IIIF Manifest', () => {
      const resource = {
        ...
        dct_references_s: JSON.stringify({
          'http://iiif.io/api/presentation#manifest': 'http://manifest.json',
        }),
      };
  ...
  describe('TabularEditor', () => {
    it('loads and displays data', async () => {
      ...
      expect(
        screen.getByText('Loading table data from DuckDB...')
      ).toBeDefined();
    ...
    it('handles refresh click', async () => {
      ...
      render(
        <TabularEditor onSelectResource={vi.fn()} onRefresh={onRefresh} />
      );
      await waitFor(() =>
        screen.getByText('No resources found.', { exact: false })
      );
