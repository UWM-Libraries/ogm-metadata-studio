...
export const App: React.FC = () => {
  ...
  const [editingDistributions, setEditingDistributions] = useState<
    Distribution[]
  >([]);
        ...
        <ToastProvider>
          <main className="flex-1 p-6 w-full mx-auto flex flex-col min-h-0">
            <div className="flex-1 flex flex-col min-h-0 space-y-6">
              <section
                className={`rounded-xl border border-gray-200 dark:border-slate-800 bg-white/50 dark:bg-slate-900/60 p-6 flex-1 flex flex-col min-h-0 shadow-sm dark:shadow-none backdrop-blur-sm ${view === 'map' ? '' : 'overflow-hidden'}`}
              >
                {(view === 'dashboard' ||
                  view === 'list' ||
                  view === 'gallery' ||
                ...
                {view === 'resource_admin' && selectedId && (
                  <div className="-m-6 h-[calc(100%+3rem)]">
                    <ResourceAdmin
                      id={selectedId}
                      onBack={() =>
                        setUrlState({ view: 'resource', id: selectedId })
                      }
                    />
                  </div>
                )}
                ...
                {view === 'import' && (
                  <div className="flex flex-col h-full">
                    ...
                    <ImportPage
                      resourceCount={resourceCount}
                      onEdit={handleEditResource}
                      onDatabaseChanged={refreshResourceCount}
                    />
                  </div>
                )}
              </section>
