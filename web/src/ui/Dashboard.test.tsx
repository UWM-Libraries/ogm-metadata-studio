...
vi.mock('./MapFacet', () => ({
  MapFacet: ({ onChange }: any) => (
    <button onClick={() => onChange({ minX: 0, minY: 0, maxX: 10, maxY: 10 })}>
      Update Map Facet
    </button>
  ),
}));
vi.mock('./TimelineFacet', () => ({
  TimelineFacet: ({ onChange }: any) => (
    <button onClick={() => onChange([2000, 2020])}>
      Update Timeline Facet
    </button>
  ),
}));
...
describe('Dashboard Component', () => {
  ...
  it('exports filtered JSON with the selected AGSL filename profile', async () => {
    (databaseService.exportFilteredResults as any).mockResolvedValue(
      new Blob(['{}'])
    );
    global.URL.createObjectURL = vi.fn(() => 'blob:url');
    global.URL.revokeObjectURL = vi.fn();
  ...
  it('handles export failure', async () => {
    (databaseService.exportFilteredResults as any).mockRejectedValue(
  ...
  it('shows actionable AGSL validation failures and opens the selected record', async () => {
    (databaseService.exportFilteredResults as any).mockRejectedValue(
      new ExportValidationError([
        {
          recordId: 'ark:-77981-gmgscj87k49',
          issue: {
            profile: 'agsl',
            field: 'dct_rights_sm',
            code: 'required',
    ...
    expect(mockOnEdit).toHaveBeenCalledWith('ark:-77981-gmgscj87k49');
  ...
  describe('Sidebar Facets', () => {
    ...
    it('toggles facet inclusion', () => {
      ...
      expect(mockToggleFacet).toHaveBeenCalledWith(
        'dct_spatial_sm',
        'Place 1',
        'include'
      );
  ...
  it('handles map hover state', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByTestId('results-map-view')).toHaveTextContent(
        /Highlight: 1/
      );
    });
    ...
    await waitFor(() => {
      expect(screen.getByTestId('results-map-view')).not.toHaveTextContent(
        /Highlight: 1/
      );
    });
  });
