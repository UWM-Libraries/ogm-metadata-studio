...
vi.mock('./queries', () => ({
  queryResources: vi
    .fn()
    .mockResolvedValue([{ id: 'res-1', dct_title_s: 'Test', extra: {} }]),
  compileFacetedWhere: vi.fn().mockReturnValue({ sql: '1=1' }),
  fetchResourcesByIds: vi
    .fn()
    .mockResolvedValue([{ id: 'res-1', dct_title_s: 'Test', extra: {} }]),
}));
...
describe('Export Logic', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    (dbInit.getDuckDbContext as any).mockResolvedValue({
      db: mockDb,
      conn: mockConn,
    });
  });
  ...
  describe('zipResources', () => {
    it('adds files to zip and returns blob', async () => {
      ...
      expect(zipInstance.file).toHaveBeenCalledWith(
        'metadata-aardvark/Maps/res-1.json',
        expect.any(String)
      );
      expect(zipInstance.file).toHaveBeenCalledWith(
        'metadata-aardvark/Uncategorized/res-2.json',
        expect.any(String)
      );
      expect(zipInstance.file).toHaveBeenCalledWith(
        'metadata-aardvark/metadata.parquet',
        parquet
      );
    ...
    it('rejects canonical ARKs where the AGSL profile requires a resource id', () => {
      expect(() =>
        exporter.jsonFilenameForResource(
          { id: 'ark:/77981/gmgs0c4sj3x' },
          { filenameProfile: 'agsl' }
        )
    ...
    it('reports identifier mismatches before generating an AGSL archive', async () => {
      ...
      await expect(
        exporter.zipResources(resources, null, {
          filenameProfile: 'agsl',
        })
      ).rejects.toThrow('ark:-77981-gmgs0c4sj3x [agsl:dct_identifier_sm]');
    ...
    it('reports records with missing ids instead of silently omitting them from an AGSL archive', async () => {
      ...
      ).rejects.toThrow('(missing id) [agsl:id]');
    });

    it('makes unsafe identifiers safe in the default profile', () => {
      expect(exporter.jsonFilenameForResource({ id: 'ark:/1234/a:b?c' })).toBe(
        'ark__1234_a_b_c.json'
      );
    });

    it('supports configurable suffix and directory policy', () => {
      expect(
        exporter.jsonPathForResource(
  ...
  describe('exportFilteredResults', () => {
    it('validates AGSL records before attempting Parquet generation', async () => {
      const queries = await import('./queries');
      vi.mocked(queries.queryResources).mockResolvedValueOnce([
        {
          id: 'ark:-77981-invalid',
          gbl_resourceClass_sm: ['Maps'],
          extra: {},
        },
      ] as any[]);
      ...
      await expect(
        exporter.exportAardvarkJsonZip({ filenameProfile: 'agsl' })
      ).rejects.toThrow('AGSL export validation failed');
      expect(mockDb.registerFileText).not.toHaveBeenCalled();
    });
